...
const DataGridContainer = ({ psiscores, lux_url, client_url }: { psiscores: Array<PSIScoreType>, lux_url:string[], client_url:string }) => {
  ...
  return (
    <div>
      <div className="p-2"><span>Lux page: </span><a href={lux_url[0]} target="_blank" className="text-blue-400">{lux_url}</a></div>
      <DataGrid columns={columns} rows={getLuxScoresOnly()} />
      <div className="p-2"><span>Client page: </span><a href={client_url} target="_blank" className="text-blue-400">{client_url}</a></div>
